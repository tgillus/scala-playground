...
object Lists:
  ...
  def sum(xs: List[Int]) =
    ...
    def add(xs: List[Int], acc: Int): Int =
      xs match
        case head :: next => add(next, acc + head)
        case Nil          => acc
  ...
  def max(xs: List[Int]) =
    @tailrec
    def find(xs: List[Int], max: Int): Option[Int] =
      xs match
        case head :: next =>
          if head > max then find(next, head) else find(next, max)
        case Nil => Some(max)

    xs match
      case head :: next => find(next, head)
      case Nil => None
